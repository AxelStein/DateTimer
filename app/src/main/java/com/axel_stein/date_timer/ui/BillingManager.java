...
public class BillingManager implements PurchasesUpdatedListener, BillingClientStateListener {
    ...
    public BillingManager(Context context, AppSettings settings) {
    ...
    public void revoke(@NonNull String productId) {
        if (billingClient == null) return;

        Purchase current = null;
        for (Purchase purchase : purchases) {
            List<String> products = purchase.getProducts();
            if (products.contains(productId) && purchase.getPurchaseState() == Purchase.PurchaseState.PURCHASED) {
                current = purchase;
                break;
            }
        }
    ...
    private void disableAds() {
        boolean adsEnabled = settings.isAdsEnabled();
